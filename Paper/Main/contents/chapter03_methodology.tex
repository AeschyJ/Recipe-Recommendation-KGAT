...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{互動圖：}若使用者 $u$ 曾與食譜 $v$ 互動（例如收藏或評分），則建立邊 $(u, \text{interact}, v)$。
  \item \textbf{食譜屬性圖：}將食譜的標籤（如 \texttt{breakfast}、\texttt{chicken}、\texttt{weeknight}）、烹飪方式、食材等屬性作為知識圖譜中的實體節點，並建立對應的關係邊。例如，「食譜 A 包含食材 chicken」表示為三元組 $(v_A, \text{has\_ingredient}, e_\text{chicken})$。
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{食材超級節點：}統計所有食材的出現頻率後，移除出現頻率排名前 0.5\% 的高頻食材（共 74 個）以及 13 個語義過於通用的基礎食材（包含 \texttt{salt}、\texttt{water}、\texttt{butter}、\texttt{sugar}、\texttt{eggs} 等）。這些食材出現在絕大多數食譜中，若納入 KG 將成為連結數萬節點的超級節點，其嵌入幾乎無法攜帶具有區辨力的語義資訊。
