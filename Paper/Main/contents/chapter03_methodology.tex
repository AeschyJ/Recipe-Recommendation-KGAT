...
本章詳述本研究所採用的模型架構與評估方法，包含系統整體架構、食譜知識圖的建構流程、KGAT 模型的數學形式化、訓練策略，以及基於忠實度指標的可解釋性評估框架。

\section{系統架構}

本研究之系統架構可分為四個階段：(1) 資料預處理與知識圖建構：將 Food.com 原始資料轉化為結構化的協同知識圖；(2) 模型訓練：在協同知識圖上訓練 KGAT 模型，學習使用者與食譜的高品質嵌入表示；(3) 推薦生成：基於學習到的嵌入，為每位使用者生成 Top-$K$ 推薦清單；(4) 可解釋性評估：透過注意力路徑萃取與忠實度指標量化，驗證推薦結果的可解釋性。
...
\subsection*{協同知識圖 (CKG) 建構}

遵循 KGAT~\cite{wang2019kgat} 的設計理念，我們將使用者-食譜互動圖與食譜知識圖統一為一個協同知識圖 $\mathcal{G}$。具體而言：

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.82\linewidth]{figures/fig_ckg_schema.pdf}
  \caption{協同知識圖 (CKG) 架構示意：整合使用者-食譜互動圖與食譜知識圖，包含互動、食材與標籤三類關係邊。}
  \label{fig:ckg_schema}
\end{figure}

\begin{enumerate}[leftmargin=2em]
  \item \textbf{互動圖：}若使用者 $u$ 在訓練集中曾與食譜 $v$ 互動（例如收藏或評分），則建立邊 $(u, \text{interact}, v)$。測試集中的互動不納入 CKG，以避免資訊洩漏。
  \item \textbf{食譜屬性圖：}將食譜的標籤（如 \texttt{breakfast}、\texttt{chicken}、\texttt{weeknight}）與食材等屬性作為知識圖中的實體節點，並建立對應的關係邊，共形成食材（\texttt{has\_ingredient}）與標籤（\texttt{has\_tag}）兩類知識關係。例如，「食譜 A 包含食材 chicken」表示為三元組 $(v_A, \text{has\_ingredient}, e_\text{chicken})$。
...
在建構知識圖三元組之前，本研究先對高頻屬性實體進行超級節點修剪，以避免過度通用的節點在訊息傳播過程中引入大量雜訊路徑。

\begin{itemize}[leftmargin=2em]
  \item \textbf{食材超級節點：}統計所有食材的出現頻率後，移除前 0.5\% 的 61 個高頻食材，以及 13 個通用基礎調味料（如 \texttt{salt}、\texttt{water}、\texttt{butter}、\texttt{sugar}、\texttt{eggs} 等）；兩者取聯集後共排除 74 個食材實體（詳細清單見附錄~\ref{app:preprocessing}）。這些食材出現在絕大多數食譜中，若納入 KG 將成為連結數萬節點的超級節點，其嵌入幾乎無法攜帶具有區辨力的語義資訊。
  \item \textbf{標籤超級節點：}統計所有標籤的出現頻率後，移除出現頻率排名前 50 的高頻標籤（如 \texttt{easy}、\texttt{healthy}、\texttt{main-dish} 等），這些標籤涵蓋面過廣，無法有效區分食譜的特色屬性。
\end{itemize}

此設計決策的理論依據在於：超級節點因與大量節點相連，其在 GNN 訊息傳播中相當於一個「資訊匯流點」，會造成鄰近節點的嵌入趨於相似（過度平滑）~\cite{li2018deeper}，並嚴重放大 §\ref{sec:kg_ablation} 所述的「淺層知識圖資訊污染」效應。修剪後的知識圖保留語義區辨力強的屬性實體，使得實體間的關聯更能反映食譜的獨特風味特色。

\section{知識圖注意力網路 (KGAT)}
...
經過 $L$ 層傳播後，為保留不同感受野下的多尺度特徵並防止表徵崩塌，本研究將各層的嵌入進行跳躍拼接 (Jumping Knowledge Connection)~\cite{xu2018jumping}：
\begin{equation}
  \bm{e}_h^* = \bm{e}_h^{(0)} \| \bm{e}_h^{(1)} \| \cdots \| \bm{e}_h^{(L)}
  \label{eq:concat}
\end{equation}

此設計能有效緩解深層圖神經網路 (GNN) 中常見的「過度平滑 (Over-smoothing)」病理現象~\cite{li2018deeper}。在深層圖傳播中，隨著網路層數 $L$ 增加，各節點因遞迴聚合鄰居資訊，其嵌入表徵容易趨於一致，導致模型喪失個體區辨力。公式~(\ref{eq:concat}) 藉由明確保留第 0 層的初始特徵 $\bm{e}_h^{(0)}$（即使用者與食譜的獨特本質特徵）並與高階拓撲上下文特徵進行拼接，確保模型在深層架構（如本研究採用的 $L=3$）下依然能維持嵌入的辨識度。此外，本研究亦在訊息聚合過程與權重矩陣上施加 Dropout 正則化，進一步抑制特徵共線性與模型過擬合 (Overfitting) 的風險。
...
\section{基於忠實度 (Fidelity) 的可解釋性評估框架}
...
\subsection*{忠實度 (Fidelity) 量化指標}

事後解釋 (Post-hoc Explanation) 的關鍵評估準則在於其保真度 (Faithfulness，或稱忠實度)——即所萃取的解釋路徑是否真正主導了模型內部的決策邏輯。為此，本研究引進 Fidelity 評估指標~\cite{agarwal2024fidelity}，透過擾動 (Perturbation) 輸入圖結構的方式，量化評估解釋路徑的「必要性 (Necessity)」與「充分性 (Sufficiency)」。

給定使用者 $u$ 與推薦物品 $v$，令 $S$ 為所萃取的解釋路徑邊集合，$p_{uv}$ 為模型對該推薦的原始預測機率（經過 Sigmoid 映射）：

\begin{itemize}[leftmargin=2em]
  \item \textbf{Fidelity$^+$（必要性忠實度）：}用於衡量解釋路徑 $S$ 是否為做出推薦的必要條件。具體做法是從協同知識圖 (CKG) 中「移除」屬於解釋路徑 $S$ 的所有關係邊，並令模型在殘缺的圖拓撲上重新進行訊息傳遞與分數預測，計算其預測機率的降幅：
    \begin{equation}
      F^+ = p_{uv} - p_{uv \setminus S}
      \label{eq:fidelity_plus}
    \end{equation}
    若 $F^+ > 0$，表示移除這些解釋邊後模型的預測信心下降，證明這組解釋邊在原始決策中具有正向的必要貢獻；$F^+$ 的數值越大，代表該解釋的必要性越高。

  \item \textbf{Fidelity$^-$（充分性忠實度）：}用於衡量解釋路徑 $S$ 是否已包含做出推薦的充分資訊。具體做法是「僅保留」屬於解釋路徑 $S$ 的關係邊，將 CKG 中的其他所有關係邊完全遮蔽 (Mask)，令模型在此極度稀疏的子圖上重新預測，計算其與原始預測的機率差值：
    \begin{equation}
      F^- = p_{uv} - p_{uv|S}
      \label{eq:fidelity_minus}
    \end{equation}
    若 $F^- \approx 0$（甚至 $F^- < 0$），表示模型僅依靠這組解釋邊所構成的極小圖結構，便能產出與原始完整圖結構相近的推薦信心，證明該解釋已涵蓋了決策所需的充分資訊；若 $F^-$ 偏高，則代表模型仍高度依賴其他未被解釋路徑覆蓋的分散式表徵。
\end{itemize}

理想的解釋路徑應同時滿足 $F^+ \gg 0$（高必要性）且 $F^- \approx 0$（高充分性）。然而需要指出的是，此類基於圖結構擾動的評估方法存在固有的學術限制：直接移除邊或遮蔽大量子圖會破壞圖的全局拓撲特性，從而引發「分佈偏移 (Distribution Shift)」問題~\cite{agarwal2024fidelity}。模型在評估階段面對其在訓練階段從未見過的殘缺圖結構，其預測行為可能產生非預期的波動。本研究在第四章的結果分析中將對此現象進行更深入的討論。
