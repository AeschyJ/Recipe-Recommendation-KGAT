...
此外，目前多數研究在 Fidelity 評估時面臨樣本量不足、僅在淺層模型上驗證等限制，深層模型（$L \geq 3$）的可解釋性驗證仍為一個尚未被充分探索的研究方向。本研究即針對此缺口，在最佳效能的 $L{=}3$ 模型上進行初步的 Fidelity 評估。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{l ccccc}
    ...
    \midrule
    \textbf{本研究}                     & \ding{51} & \ding{51} & 初步  & \textbf{\ding{51}} & \textbf{\ding{51}} \\
    \bottomrule
  \end{tabular}%
  }
\end{table}
