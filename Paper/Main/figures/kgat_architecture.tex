...
\begin{scope}[on background layer]
    ...
    \node[anchor=north west, font=\sffamily\bfseries, text=slate, xshift=5pt, yshift=-5pt] at (input_frame.north west) {1. 輸入層：協同知識圖 (CKG)};
\end{scope}
...
\node[module, below=1cm of concat, minimum width=6cm] (pred) {內積運算層\\預測最終使用者偏好分數};
